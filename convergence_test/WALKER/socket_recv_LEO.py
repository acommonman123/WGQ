...
def create_raw_socket():
    try:
        # 创建IPv6原始套接字
        raw_socket = socket.socket(socket.AF_INET6, socket.SOCK_RAW, socket.IPPROTO_RAW)
        return raw_socket
...
def dealwith_route_dblen(sock, src_ip, db_len, lasthop, ttl):
    ...
    if int(db_len) - self_db_length >4: 
...
def periodicly_request_incluster(sock, node_id, src_ip, head_id, dest_ip, local_ips, local_ip_len, route_to_head):
    while True:
        time.sleep(3)
        if waiting_incluster_conform.value == 1:
            print("resending in_cluster")
            send_in_cluster(sock, node_id, src_ip, head_id, dest_ip, local_ips, local_ip_len, route_to_head)
        else:
            break
    ...
    try:
        while True:
            ...
            if payload_parts[0] == "incluster_inform":
                ...
                last_hop = addr[0]
                if head_ip != addr[0] and get_nexthop_ip(head_ip) == None:
                ...
                if status == "HEAD":
                    ...
                    #status = "UNDETERMINED"
                    #waiting_incluster_conform.value = 1
                    if incluster_time != float('inf') and time.time() - incluster_time < 3: #如果在启动60秒内，忽略入簇通告
                        continue
                    ...
                    #periodic_process = multiprocessing.Process(target=periodicly_request_incluster, args=(raw_socket, NODE_ID, src_ip, head_id, head_ip, local_ips, str(local_ip_len), route_to_head))
                    #periodic_process.start()
                    #periodicly_request_incluster(raw_socket, NODE_ID, src_ip, head_id, head_ip, local_ips, str(local_ip_len), route_to_head)
                ...
                if ttl > 1:
                    ...
                    if CLUSTER_ID == cluster_id:
                        forward_incluster_inform(raw_socket, filtered_neighbor_list, src_ip, head_id, cluster_id, head_ip, str(ttl), route_to_head)
            ...
            if payload_parts[0] == 'head_inform':
                ...
                hello_process.start()
                update_intercluster_routing_process = multiprocessing.Process(target=periodic_update_intercluster_routing, args=(raw_socket, src_ip, 8))
            ...
            if payload_parts[0] == "incluster_confirm":
                ...
                if member_id == NODE_ID and status == "IDLE":
                    ...
                    HEAD_IP = head_ip
                    status = 'MEMBER'
                    waiting_incluster_conform.value = 0
...
if __name__ == "__main__":
    ...
    waiting_incluster_conform = manager.Value('i', '0')
    waiting_incluster_conform.value = 0
