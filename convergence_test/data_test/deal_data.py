...
def farward_data(dest_ip, src_ip, data_length, delay, shared_queue):
    ...
    while True:
        try:
            ...
            print(f"data forward to {nexthop_ip}")
            break
        except socket.error as e:
            print(f"Error forwarding in_cluster packet: {e}")
            time.sleep(0.2)
    shared_queue.value -= 1
...
def deal_data(raw_socket, shared_queue):
    
    src_ip = get_src_ip()
    ip_addr_output = subprocess.check_output(['ip', '-6', 'addr'], text=True)
    ip_addresses = re.findall(r'inet6\s+([0-9a-fA-F:]+)(?:/\d+)?', ip_addr_output)
    local_ips = [ip for ip in ip_addresses if ip.startswith(('fd00:'))]  # 过滤掉2001和fe80开头的地址
    while True:
        ...
        if payload_parts[0] == "data":
            dest_ip = payload_parts[1]
            data_length = payload_parts[2]
            delay = float(payload_parts[3])
            if dest_ip not in local_ips:
                if shared_queue.value < 10:
                    shared_queue.value += 1
                    farward_process = multiprocessing.Process(target=farward_data, args=(dest_ip, src_ip, data_length, delay, shared_queue))
                    farward_process.start()
                    #farward_data(dest_ip, src_ip, data_length, delay, shared_queue)
            else:
                print("data arrived at destination")
                with open('/files/arrived_pkts.txt', 'a') as f:
                    f.write(f"{delay}\n")

if __name__ == "__main__":
    NODE_ID = sys.argv[1]
    raw_socket = create_raw_socket()
    queue = multiprocessing.Manager().Value('i', 4)
    if NODE_ID in congestion_nodes:
        queue.value = 9
    deal_data(raw_socket, queue)