...
def incluster_inform(sock, neighbor_ip_list, source_ip, seq):
    hop_limit = '5'
    data = b"incluster_inform " + sys.argv[1].encode('ascii') + b" " + sys.argv[2].encode('ascii') + b" " + source_ip.encode('ascii') + b" " + hop_limit.encode('ascii')#head_inform <node_id> <cluster_id> <head_ip> <hop_limit>
    data_len = len(data)
    # 发送数据包
...
def main():
    ...
    if raw_socket is None:
        return
    while True:
        incluster_inform(raw_socket, neighbor_ip_list, src_ip, str(seq))
        seq += 1
