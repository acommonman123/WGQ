...
def update_cluster_neighbor(payload_parts, sock, src_ip, lasthop):
    ...
    cluster_neighbor_update_MEO(sock, update_type, src_ip, received_cluster_id, received_cluster_neighbor)
...
def cluster_neighbor_update_MEO(sock, update_type, src_ip, cluster_id, cluster_neighbor):
    global head_ip_list
    for dest_ip in head_ip_list:
        data = b"cluster_neighbor_update " + update_type.encode('ascii') + b" " + str(cluster_id).encode('ascii') + b" " + cluster_neighbor.encode('ascii') # cluster_update <cluster_id> <ttl> <seq> <cluster_member_list>
        data_len = len(data)
        # 创建 IP 头部
        ip_header = create_ipv6_header(src_ip, dest_ip, data_len, hop_limit=64)
            ...
            if payload_parts[0] == "cluster_update":
                ...
                if status == 'MEO':
                    ...
                    with open('log.txt', 'a') as f:
                        f.write(f"cluster_udpate messate:{' '.join(received_member_list)}, cluster_id:{cluster_id_received},seq: {seq}\n")
