...
def call_hello(node_id, cluster_id, neighbor_ip_list, source_ip, local_ips):
    while True:
...
def call_incluster_inform(sock, neighbor_ip_list, source_ip, node_id, cluster_id):
    while True:
        hop_limit = '3'
        data = b"incluster_inform " + node_id.encode('ascii') + b" " + cluster_id.encode('ascii') + b" " + source_ip.encode('ascii') + b" " + hop_limit.encode('ascii')#head_inform <node_id> <cluster_id> <head_ip> <hop_limit>
        data_len = len(data)
        # 发送数据包
        for dest_ip in neighbor_ip_list:
            k = 0
            while k < 5:
                try:
                    ...
                    print(f'Error sending incluster_inform to {dest_ip}')
...
def cluster_neighbor_update(sock, src_ip, cluster_id, cluster_neighbor):
    ...
    data = b"cluster_neighbor_update " + cluster_id.encode('ascii') + b" " + cluster_neighbor.encode('ascii') + b" " + seq.encode('ascii')  # cluster_neighbor_update <cluster_id> <neighbor_list>
    data_len = len(data)
    # 创建 IP 头部
    ip_header = create_ipv6_header(src_ip, MEO_ip, data_len, hop_limit=64)
...
def update_node_state(payload_parts,sock, src_ip):
    ...
    with lock:
        ...
        if update_type == 'NEW':
            ...
            if node_id in ['WALKERdgm-3101', 'WALKERdgm-3201', 'WALKERdgm-4001', 'WALKERdgm-3103', 'WALKERdgm-3104', 'WALKERdgm-3111', 'WALKERdgm-3203', 'WALKERdgm-3204', 'WALKERdgm-3211', 'WALKERdgm-4003', 'WALKERdgm-4004', 'WALKERdgm-4011']:
                intra_cluster_graph[node_id].append((neighbor_id, 5*weight))
            else:
...
def dealwith_route_dblen(sock, src_ip, db_len, lasthop, ttl):
    ...
    if int(db_len) - self_db_length >5: 
...
def update_cluster_neighbor(payload_parts):
    ...
    with lock:
        ...
        if received_cluster_id != CLUSTER_ID:
            if received_cluster_id == "1":
                inter_cluster_graph[received_cluster_id].append((received_cluster_neighbor, 5*weight))  # 添加当前节点到邻簇图中
            else:
                inter_cluster_graph[received_cluster_id].append((received_cluster_neighbor, weight))
...
def dealwith_new_cluster_neighbor(raw_socket, src_ip, payload_parts):
    ...
    if new_cluster_neighbor_id not in list(cluster_edge_list.keys()):
        ...
        with lock:
            ...
            if CLUSTER_ID == "1":
                inter_cluster_graph[CLUSTER_ID].append((new_cluster_neighbor_id, 5*weight))
            else:
...
def dealwith_incluster(payload_parts, raw_socket, src_ip, seq):
    '''global member_list
    global member_ip_list
    global cluster_list'''
    global NODE_ID
    ...
    cluster_update_head(raw_socket, src_ip, CLUSTER_ID, " ".join(member_ips), '2', seq)
    ...
    try:
        while True:
            ...
            if payload_parts[0] == "hello":
                ...
                if neighbor_cluster_id == CLUSTER_ID and neighbor_id not in neighbor_id_list and status == 'MEMBER':
                    with lock:
                        neighbor_list[neighbor_id] = neighbor_ip
                        neighbor_id_list.append(neighbor_id)
                    neighbors = " ".join(neighbor_id_list)
            ...
            if payload_parts[0] == "new_cluster_neighbor":
                if payload_parts[1] in list(cluster_edge_list.keys()):
                    with lock:
                        last_update_intracluster_time.value = time.time()
            ...
            if payload_parts[0] == "node_update":
                ...
                process = multiprocessing.Process(target=update_node_state, args=(payload_parts, raw_socket, src_ip))
                process.start()
            ...
            if payload_parts[0] == 'head_inform':
                ...
                incluster_inform_process = multiprocessing.Process(target=call_incluster_inform, args=(raw_socket,neighbor_ip_list, src_ip, NODE_ID, CLUSTER_ID))
                incluster_inform_process.start()
                hello_process = multiprocessing.Process(target=call_hello, args=(NODE_ID, CLUSTER_ID, neighbor_ip_list, src_ip, local_ips))
                hello_process.start()
                update_intercluster_routing_process = multiprocessing.Process(target=periodic_update_intercluster_routing, args=(raw_socket, src_ip, 6))
                update_intercluster_routing_process.start()
                update_intracluster_routing_process = multiprocessing.Process(target=periodic_update_intracluster_routing, args=(raw_socket, src_ip, 4))
                update_intracluster_routing_process.start()
                cluster_update_process = multiprocessing.Process(target=cluster_update_head, args=(raw_socket, src_ip, CLUSTER_ID, " ".join(ip_addresses), '2', str(cluster_update_seq.value)))
                cluster_update_seq.value += 1
                cluster_update_process.start()
                with open('/files/cluster_result.txt', 'a') as f:
                    result = f"{NODE_ID},{CLUSTER_ID},{HEAD_ID},{status}\n"
                    f.write(result)
                '''with open("output.txt", "a") as file:
            ...
            if payload_parts[0] == "in_cluster":
                ...
                if NODE_ID == payload_parts[2]:
                    #print('new cluster member')
                    seq = cluster_update_seq.value
                    process = multiprocessing.Process(target=dealwith_incluster, args=(payload_parts, raw_socket, src_ip, str(seq)))
                    process.start()
                    cluster_update_seq.value += 1
            ...
            if payload_parts[0] == "incluster_confirm":
                ...
                if member_id == NODE_ID and status == "IDLE":
                    HEAD_ID = head_id
            ...
            if payload_parts[0] == "cluster_update":
                ...
                if cluster_id_received == CLUSTER_ID and status == "MEMBER":
...
'''                
                ...
                if cluster_id_received != CLUSTER_ID and status == "HEAD":
                    last_update_intercluster_time.value = time.time()
