...
db = pymysql.connect(host='10.101.167.96',
                     user='root',
                     password='123456'
                     )
cursor = db.cursor()
...
def get_convergence_time():
    '''temp_db = pymysql.connect(host='10.101.167.96',
                     user='root',
                     password='123456'
                     )
...
def cluster_routing():
    os.system('~/anaconda3/envs/be2/bin/python /home/wspn02/mega/WGQ/convergence_test/baseline/cluster_routing.py')

if __name__ == "__main__":
    route_monitor_list = []
    df = pd.read_csv('/home/wspn02/mega/WGQ/LXM/walker_dyd.csv')
    name_list = df['satellite name'].tolist()
    container_list = random.sample(name_list, 100)
    route_monitor_list = []
