...
def publisher():
    # 连接到本地 Redis 服务器
    r = redis.Redis(host='10.101.167.96', port=6379, db=0)
...
def delay(delay, container, interface):
    os.system("docker exec -d %s tc qdisc add dev eth-%s root netem delay %sms" %(container, interface, delay))
    print("docker exec -d %s tc qdisc add dev eth-%s root netem delay %sms" %(container, interface, delay))
