...
def dijkstra(Graph, start, end):
    ...
    start, end: node identifiers
    返回: (path_list, distance)  若不可达返回 ([], float('inf'))
    注意: 需要保证所有边权 >= 0
    """
    # 基本合法性检查
    ...
    # 常见的做法：使用 stale-check (d > dist[u]) 跳过过时条目
    while pq:
        d, u = heapq.heappop(pq)
        # 如果这是一个过时的条目（后来发现了更短的距离），跳过
        if d > dist[u]:
            continue
        ...
        # 保护：Graph 中应包含 u（按上面检查应当始终包含）
        for v, w in Graph[u]:
            if w < 0:
                raise ValueError("Dijkstra requires non-negative edge weights")
            # 松弛操作：使用 dist[u]（而不是弹出的 d, 虽然 d == dist[u] 通常成立）
            nd = dist[u] + w
