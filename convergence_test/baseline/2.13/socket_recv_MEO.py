...
def periodic_update_intracluster_routing(sock, src_ip, interval, shared_is_graph_updated):
    ...
    while True:
        try:
            ...
            if shared_is_graph_updated.value == 'True':
                shared_is_graph_updated.value = 'False'
...
def send_del_node_neighbor(sock, src_ip, node_id, neighbor_id, link_subnet, cluster_id):
    data = b"del_node_neighbor " + node_id.encode('ascii') + b" " + neighbor_id.encode('ascii') + b" " + link_subnet.encode('ascii') + b" " + cluster_id.encode('ascii') # del_node_neighbor <node_id> <neighbor_id> <link_subnet>
    data_len = len(data)
    for dest_ip in neighbor_ip_list:
        # 创建 IP 头部
        ip_header = create_ipv6_header(src_ip, dest_ip, data_len, hop_limit=64)
        ...
        while True:
            try:
                ...
                with open("log.txt", "a") as file:
                    result = f"Node {NODE_ID} sent del_node_neighbor to {dest_ip}, node_id: {node_id}, neighbor_id: {neighbor_id}, link_subnet: {link_subnet}\n"
                    file.write(result)
                break
            except socket.error as e:
                print(f"Error sending del_node_neighbor packet: {e}")
                with open('error_log.txt', 'a') as log_file:
                    log_file.write(f"Error sending packet to {dest_ip}: {e}\n")
                time.sleep(0.2)

def forward_del_node_neighbor(data, sock, src_ip, lasthop):
    data_len = len(data)
    for dest_ip in neighbor_ip_list:
        if dest_ip == lasthop:
            continue
        ...
        while True:
            try:
                sock.sendto(packet, (dest_ip, 0))
                with open("log.txt", "a") as file:
                    result = f"Node {NODE_ID} forwarded del_node_neighbor to {dest_ip}, data: {data.decode('ascii')}\n"
                    file.write(result)
                break
            except socket.error as e:
                print(f"Error sending del_node_neighbor packet: {e}")
...
def delete_route(sock, src_ip, link_subnet):
    for member in member_list:
        member_ip = member_ip_dict[member]
        data = b"delete_route " + link_subnet.encode('ascii') # delete_route <link_subnet>
        data_len = len(data)
        # 创建 IP 头部
        ip_header = create_ipv6_header(src_ip, member_ip, data_len, hop_limit=64)
        ...
        while True:
            try:
                sock.sendto(packet, (member_ip, 0))
                with open("log.txt", "a") as file:
                    result = f"Node {NODE_ID} sent delete_route to {member_ip}, link_subnet: {link_subnet}\n"
                    file.write(result)
                break
            except socket.error as e:
                print(f"Error sending delete_route packet: {e}")
                with open('error_log.txt', 'a') as log_file:
                    log_file.write(f"Error sending packet to {member_ip}: {e}\n")
                time.sleep(0.2)
        ...
        while True:
            ...
            if payload_parts[0] == "node_update":
                ...
                if update_type == "NEW":
                    ...
                    with open("log.txt", "a") as file:
                        result = f"Received node_update DEL for node_id: {node_id}, neighbor_id: {neighbor_id} at {time.time()}\n"
                        file.write(result)
                    if node_id in list(intra_cluster_graph.keys()):
                        if (neighbor_id, 10) in intra_cluster_graph[node_id]:
                            intra_cluster_graph[node_id].remove((neighbor_id, 10))
                        if (neighbor_id, 10) in global_graph[node_id]:
                            global_graph[node_id].remove((neighbor_id, 10))
                        #send_del_node_neighbor(raw_socket, src_ip, node_id, neighbor_id, link_subnet)
                        link_subnet = payload_parts[4]
                        multiprocessing.Process(target=send_del_node_neighbor, args=(raw_socket, src_ip, node_id, neighbor_id, link_subnet, CLUSTER_ID)).start()
                    #delete_route(raw_socket, src_ip, link_subnet)
                    multiprocessing.Process(target=delete_route, args=(raw_socket, src_ip, link_subnet)).start()
                with lock:
                    is_graph_updated.value = 'True'

            if payload_parts[0] == "del_node_neighbor":
                node_id = payload_parts[1]
                neighbor_id = payload_parts[2]
                link_subnet = payload_parts[3]
                received_cluster_id = payload_parts[4]
                if received_cluster_id == CLUSTER_ID:
                    continue
                if node_id in list(global_graph.keys()):
                    if (neighbor_id, 10) in global_graph[node_id]:
                        global_graph[node_id].remove((neighbor_id, 10))
                message = " ".join(payload_parts)
                lasthop = addr[0]
                #forward_del_node_neighbor(message.encode('ascii'), raw_socket, src_ip, lasthop)
                multiprocessing.Process(target=forward_del_node_neighbor, args=(message.encode('ascii'), raw_socket, src_ip, lasthop)).start()
                #delete_route(raw_socket, src_ip, link_subnet)
                multiprocessing.Process(target=delete_route, args=(raw_socket, src_ip, link_subnet)).start()
                with lock:
                    is_graph_updated.value = 'True'
...
def main():
    ...
    periodic_process = multiprocessing.Process(target=periodic_update_intracluster_routing, args=(raw_socket, src_ip, 10, is_graph_updated))
...
if __name__ == "__main__":
    ...
    is_graph_updated = manager.Value('c_char_p', 'False')  # 标志位，指示图是否更新 
    is_graph_updated.value = 'False'
