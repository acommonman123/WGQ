...
def node_update_del(src_ip, node_id, update_type, neighbor_id, link_subnet, meo_ip):
    sock = create_raw_socket()
    data = b"node_update " + node_id.encode('ascii') + b" " + update_type.encode('ascii') + b" " + neighbor_id.encode('ascii') + b" " + link_subnet.encode('ascii') # Node_status <node_id> NEW/DEL <neighbor_id> <seq>
    data_len = len(data)
    # 创建 IP 头部
    ip_header = create_ipv6_header(src_ip, meo_ip, data_len, hop_limit=64)
    ...
    while True:
        try:
            sock.sendto(packet, (meo_ip, 0))
            print('sent node_update(del) to MEO')
            break
        except socket.error as e:
            print(f"{NODE_ID} Error sending node_update_del packet to MEO(IP: {meo_ip}): {e}")
            with open('error_log.txt', 'a') as f:
                result = f"Error sending node_update_del packet to MEO: {e}\n"
                f.write(result)
            time.sleep(0.2)
    #print("node_update sent successfully")
...
class NeighborCounter:
    def __init__(self, seconds: float, neighbor_id, socket, link_subnet, meo_ip):
    ...
    def _on_finish(self):
        ...
        with open('/files/time.txt', 'a') as f:
            ...
            f.write(result)
        event_a(self._socket, self._id, self._link_subnet, self._meo_ip)

# 断链后的行为，删除对应的邻居节点和簇邻居节点信息
def event_a(socket, id, link_subnet, meo_ip):
    try:
    ...
    try:
        ...
        print(f"{NODE_ID} neighbor_list delete error: {e}, id: {id}")
    node_update_del(src_ip, NODE_ID, 'DEL', id, link_subnet, meo_ip)
...
def delete_route(subnet):
    os.system('ip -6 route del ' + subnet)
    print(f"{NODE_ID} Route to {subnet} deleted.")
    with open("del_route_log.txt", "a") as file:
        result = f"Route to {subnet} deleted."
        file.write(result + '\n')
    ...
    try:
        while True:
            ...
            if payload_parts[0] == "hello":
                ...
                if neighbor_id in list(neighbor_counter.keys()):
                ...
                if neighbor_id not in neighbor_id_list and status == 'MEMBER':
                    ...
                    neighbors = " ".join(neighbor_id_list)
                    node_update(raw_socket, src_ip, NODE_ID, 'NEW', neighbor_id)
                    cd = NeighborCounter(10, neighbor_id, raw_socket, neighbor_ip.split('::')[0] + '::/64', MEO_ip)
            ...
            if payload_parts[0] == 'delete_route':
                subnet = payload_parts[1]
                delete_route(subnet)
...
if __name__ == "__main__":
    ...
    neighbor_counter = {}     #邻居计数器，形式为{neighbor_id: NeighborCounter对象}
