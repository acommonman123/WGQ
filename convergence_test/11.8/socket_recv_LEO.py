...
def generate_intercluster_routing_table(local_inter_graph, local_intra_graph, sock, src_ip, local_inter_cluster_links):
    ...
    for end in list(local_inter_graph.keys()):
        ...
        if cost == float('inf'):
            print(f"簇{CLUSTER_ID} 到簇 {end} 不可达")
            with open('inter_cluster_graph.txt', 'a') as f:
                result = local_inter_graph.__str__() + '\n'
                f.write(result)
            continue
...
def periodic_update_intercluster_routing(sock, src_ip, interval):
    ...
    while True:
        try:    
            ...
            intercluster_routing_process = multiprocessing.Process(target=generate_intercluster_routing_table, args=(local_inter_graph, local_intra_graph, sock, src_ip, inter_cluster_links))
    ...
    try:
        while True:
            ...
            if payload_parts[0] == "hello":
                ...
                #如果是簇边缘节点接收到临簇的包, 告知簇首邻簇信息
                if neighbor_cluster_id != CLUSTER_ID and neighbor_id not in neighbor_id_list and status == "MEMBER" and neighbor_cluster_id not in cluster_neighbor_list:
                    #print('neighbor_cluster_id: ', neighbor_cluster_id)
                    with lock:
                        neighbor_list[neighbor_id] = neighbor_ip
                        neighbor_id_list.append(neighbor_id)
            ...
            if payload_parts[0] == 'head_inform':
                ...
                hello_process.start()
                update_intercluster_routing_process = multiprocessing.Process(target=periodic_update_intercluster_routing, args=(raw_socket, src_ip, 5))
                ...
                cluster_update_process = multiprocessing.Process(target=cluster_update_head, args=(raw_socket, src_ip, CLUSTER_ID, " ".join(ip_addresses), '2', '1'))
